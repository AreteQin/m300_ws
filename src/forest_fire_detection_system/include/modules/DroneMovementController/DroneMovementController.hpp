...
class DroneMovementController {
 public:
  ...
  bool ctrlDroneMOveByOffsetDestination(const float Des_x, const float Des_y, const float Des_z, const float Des_ya,
                                        float PTM, float YTD);
  ...
  bool ctrlDroneLand() {
    ...
    ROS_INFO_STREAM("Landing request sending ... need your confirmation on the remoter!");
    task_control_client.call(control_task);
    if (control_task.response.result == true) {
