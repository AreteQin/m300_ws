...
void FFDS::APP::SingleFirePointTaskManager::initMission(dji_osdk_ros::InitWaypointV2Setting *initWaypointV2SettingPtr) {
    ...
    for (int i = 0; i < LocalPosVec.size(); ++i) {
        GPSplanWriter.write(WpV2Vec[i].longitude, WpV2Vec[i].latitude, WpV2Vec[i].relativeHeight);
...
//    int isPotFireNum = 0;
//    while (ros::ok() && (waypoint_V2_mission_state_push_.state != 0x6)) { // 0x6 == exit waypointV2 mission
//        ros::spinOnce();
...
//                PRINT_ERROR(
//                        "failed to pause the mission, please use remoter to cancel!");
//                break;
//            }
//
//            PRINT_INFO(
